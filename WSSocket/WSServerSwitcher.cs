    ...
    public class WSServerSwitcher: AbstractSwitcher
    {
        public override bool Execute()
        {
            if (base._Data != null)
            {
                ...
                if (handShakeText.IndexOf("GET /chat HTTP/1.1") != -1)
                {
                    Console.WriteLine(handShakeText);
                    base.PostBack(PackHandShakeData(GetSecKeyAccetp(base._Data, base._Data.Length)));
                }
                else
                {
                    IPEndPoint endremotepoint = (System.Net.IPEndPoint)base._Client.RemoteEndPoint;
                    string IPAddress = endremotepoint.Address.ToString();
                    int Port = endremotepoint.Port;

                    Console.WriteLine("数据来源：{0} 端口：{1} 数据体：{2}", IPAddress, Port, AnalyticData(base._Data, base._Data.Length));
                }
            }
            return true;
        }
        ...
        private static string AnalyticData(byte[] recBytes, int recByteLength)
        {
            ...
            Console.WriteLine("数据体长度：{0}，{1}", payload_len, recBytes[1]);
